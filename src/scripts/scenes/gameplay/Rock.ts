import { ROCK_VELOCITY_X } from "../../utils/GameConstants";
...
/**
 * @class Rock
 * @description Creates the matter image of rock and adds it to the respective scene.
 * @extends Phaser.Physics.Matter.Image
 */
export class Rock extends Phaser.Physics.Matter.Image {
  /**
   * @constructor
   * @description Create a new instance of this class.
   * @param {RockConfig} config Configuration interface.
   */
  constructor(config: RockConfig) {
    super(
      config.world,
      config.x,
      config.y,
    ...
    this.setName(config.name); // Set name for Rock object.
    this.setMass(100); // Set mass for the game object's matter body.
    this.setIgnoreGravity(true); // Set Rock to ignore gravity.
    ...
    if (this.scene) {
      ...
      if (plane) {
        ...
        if (plane.state === states.fly) {
          this.setVelocityX(ROCK_VELOCITY_X); // Plane is flying, therefore set the 'x' velocity.
        } else {
          this.setStatic(true); // Plane is not flying, therefore make the Rock immovable.
        }
      }
    }
